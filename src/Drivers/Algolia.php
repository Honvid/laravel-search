...
class Algolia extends DriverBase
{
    /**
    ...
    protected function getClient()
    {
        if ( ! static::$client) {
            static::$client = new Client(
                config('search.connections.algolia.config.application_id'),
                config('search.connections.algolia.config.admin_api_key')
            );
        }
    ...
    public function runCount($query)
    {
        ...
        try {
            return array_get($this->getIndex()->search(array_get($query, 'terms'), array_get($query, 'query')),
                'nbHits');
        } catch (\Exception $e) {
            return 0;
        }
